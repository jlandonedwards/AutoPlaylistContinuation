...
class Metrics():
    ...
    def calculate_metrics(self,rec_tracks,rec_artists,y_tracks,y_artists):
        ...
        dcg = 1/tf.math.reduce_sum(tf.experimental.numpy.log2(tf.cast(tf.RaggedTensor.from_value_rowids(idxs_3d[:,2],idxs_3d[:,0],nrows=50),tf.float32).to_tensor()+2),1)
        idcg = 1/tf.math.reduce_sum(tf.experimental.numpy.log2(tf.cast(tf.RaggedTensor.from_value_rowids(idxs_3d[:,1],idxs_3d[:,0],nrows=50),tf.float32).to_tensor()+2),1)
        ndcg = tf.math.reduce_mean(dcg/idcg)
